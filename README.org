...
- /emm/ = unit's identifying code. It concatenates the state's /edo/ abbreviation (then a hyphen) two digits for the election cycle (then a period) and the /inegi/ or the district identifier, depending on the level of observation. Using /emm/ as sort criterion returns a state-time-unit ordering.
