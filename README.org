...
- [[./data/aymu1989-present.csv][~data/aymu1970-on.csv~]] = updated to 2022, can be processed with code/ay.r in order to systematize coalitions (ie., aggregate votes when member parties' returns are reported separately and remove redundant columns).
  - Inspect data online in Google sheets (basic instructions [[instrucciones][here]]): 
...
/Note/: data split by approximate decades to comply with Google's maximum spreadsheet size (400k cells); files have identical columns to ease appending one another off Google drive.
...
- [[./data/aymu1989-present.coalAgg.csv][~data/aymu1970-on.coalAgg.csv~]] = pre-processed version of the above so that coalition votes appear properly aggregated.
- [[./data/aymu1989-present.coalSplit.csv][~data/aymu1970-on.coalSplit.csv~]] = pre-processed version of the above (starting in 1989) so that coalition votes appear properly split among its members.
