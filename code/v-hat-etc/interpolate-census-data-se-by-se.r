...
##############################################################
sum.split <- function( d=censo, year.var=2020, rnd=1, dfull=censo, year.to.split=c(NA,2005,2010,2020)[1] ) {
    ...
    if (is.na(year.to.split)==FALSE) return(dfull)
}
##
...
nm     <- censo
nm.raw <- censo # backup, to restore
