...
Municipal election vote returns are distributed as a large cross-section time-series. Data is stored as text in comma-separated-values (\verb|csv|) format, easily accessible for inspection with spreadsheet software such as Microsoft Excel or open-source Libreoffice Calc.\footnote{Text is stored in UTF-8 character encryption. Care has been taken to drop most Latin American special characters, such as accented vowels, from text strings. The letter \~N remains, and may appear as a garbled character in machines with other default encryption systems. See \url{https://docs.python.org/3/howto/unicode.html} for a primer on character encryption.} In the resulting data matrix, each row is a municipality in a given electoral year. Columns are data fields, including unit identifiers (the municipality's name, state, census bureau ID, an so forth), the election date, and the vote total that each party or coalition won.\footnote{The repository's \verb|README.md| file includes a detailed codebook describing what is in each spreadsheet column/field.}

Vote returns appear in column pairs for each party or coalition: columns \verb|v01, v02, ...| report raw vote totals, columns \verb|l01, l02, ...| report the label of the party the votes belong to. Figure \ref{F:scrn} offers a glimpse for the state con Baja California's five municipalities over three election cycles. In Ensenada 1998, $v01 = 30660$, $l01 = pan$, $v02 = 31951$ and $l02 = pri$ reports that the National Action Party (PAN) won 30,660 votes, slightly below the PRI's 31,951 votes. The vote/label pairs can accommodate heterogeneous state party systems over four decades of high mutability  

The vote/label pairs accommodates state party system heterogeneity over four decades of high mutability, and ephemerous electoral coalitions in a relatively limited number of columns.

Vote returns appear in columns \verb|v01, v02, ...| and \verb|l01, l02, ...|, which are paired. Columns \verb|v..| report raw vote totals, columns\verb|l..| report the party or coalition label the votes belong to. So a hypothestical municipality where the PAN won 1,500 and PRI State party systems over four decades are too heterogeneous
...
Figure \ref{F:vpcts} summarizes municipal elections since 1979. Plot shares PAN PRI PRD Morena Other each year: tiny points, mean and 80band 
