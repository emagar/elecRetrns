...
\item the total effective vote (field \texttt{efec}), defined as the sum of all votes cast in the municipality that year (\texttt{tot}) excluding write-in votes (\texttt{nr})\footnote{Write-in candidate votes (for \emph{candidatos no registrados}) are considered void in Mexican electoral law.} and invalid ballots (\texttt{nul}), which is necessary to compute valid vote shares; and
...
Figure \ref{F:scrn} provides a glimpse of the dataset for five municipalities of Baja California across three electoral cycles: \verb|yr=|1998, 2001, and 2004. Municipal vote returns are stored in column pairs such as \verb|(v01,l01)|, \verb|(v02,l02)|, an so forth, corresponding to the first reported candidate, the second candidate, and so on. Within each pair, the \verb|l| component identifies the party or coalition label, while the \verb|v| component records the total votes won by the party or coalition. For example, in Ensenada's 1998 election, the PRI won a plurality of 31,995 votes ($\texttt{l02} = \texttt{pri}$ indicates that the PRI's vote total is stored in field $\texttt{v02}$), followed closely by the PAN with 30,660 votes (stored in field $\texttt{v01}$).
...
\item[edo] = state abbreviation (may differ from commonly used abbreviations, so that sorting them alphabetically preserves the order set by edon, eg. Chiapas is `cps' instead of the usual `chis').
\end{description}
\subsubsection{Temporal identification}
\begin{description}
\item[date] = date of the election, 8-digit numeric yyyymmdd. 
...
\item[v01, v02, ...] = raw vote for candidate 1, 2, etc. In the coalition-aggregated dataset only, raw vote variables are sorted in decreasing order: v01 reports the votes won by the most-voted candidate, v02 the votes won by the runner-up, and so forth.
\item[l01, l02, ...] = string label of candidate 1's, 2's, ... party or coalition.
...
\item[efec] = effective vote total, equal the sum of raw votes minus votes for write-in candidates minus invalid ballots. This is the common denominator to compute vote shares.
...
\item[race.after] = incumbent's status in the next consecutive race, reporting what occured at the end of the current term. It takes one of the following categories:
  \begin{itemize}
...
\item[status] = string variable for housekeeping. It takes one of the following categories:
  \begin{itemize}
  \item `ok' = returns have been revised, no issue reported;
  \item `new' = new municipality electing authorities for the first time;
  \item `missing' = data unavailable;
  \item `appointed' = state assembly appointed a Concejo Municipal (municipal council) due to post-election civil strife or some other circumstance impeding celebrating an election;
...
\item[ddied, drepe, drepg, dlegacy, who] = experimental variables with development in progress but insufficient for use for analysis.
\end{description}
...
\item[dconcgob]  = dummy equal 1 if the municipal election concurred with a gubernatorial election, 0 otherwise. 
\item[dconcpres] = dummy equal 1 if the municipal election concurred with a presidential election, 0 otherwise. 
\item[ddied] = experimental variable with development in progress but insufficient for use for analysis.
\item[dextra] = dummy equal 1 for special elections (elección extraordinaria), 0 otherwise.
\item[dlegacy] = experimental variable with development in progress but insufficient for use for analysis.
\item[dmujer] = dummy equal 1 if incumbent is a woman, 0 otherwise. 
\item[drepe] = experimental variable with development in progress but insufficient for use for analysis.
\item[drepg] = experimental variable with development in progress but insufficient for use for analysis.
...
\item[tot] = total raw votes.
\item[v01, v02, ...] = raw vote for candidate 1, 2, etc. In the coalition-aggregated dataset only, raw vote variables are sorted in decreasing order: v01 reports the votes won by the most-voted candidate, v02 the votes won by the runner-up, and so forth.
\item[who] = experimental variable with development in progress but insufficient for use for analysis.
\item[win] = winner's party or coalition.
