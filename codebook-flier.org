...
** Unit IDs
\begin{description}
\item[emm] = unit's unique identifying code. It concatenates the state's /edo/ abbreviation (then a hyphen), two sequential digits for the electoral cycle (then a period) and the /inegi/ identifier. Voided elections followed by a special election with non-missing data have an `a' inserted after the electoral cycle. Using /emm/ as sort criterion returns a state-time-unit ordering.
...
\begin{description}
- \item[dconcpres] = dummy equal 1 if the municipal election is concurrent with a presidential election, 0 otherwise. 
- \item[dconcdipf] = dummy equal 1 if the municipal election is concurrent with a congressional election, 0 otherwise. 
- \item[dconcgob] = dummy equal 1 if the municipal election is concurrent with a gubernatorial election, 0 otherwise. 
- \item[dconcdipl] = dummy equal 1 if the municipal election is concurrent with a state assembly election, 0 otherwise. 
\end{description}
...
\item[seccion] = voting precinct identifier = /edon/ * 10000 + sección electoral numeral (as set by IFE/INE).
\item[tot] = total raw votes.
\item[v01, v02, ...] = raw vote for candidates 1, 2, etc.
\item[vhat.pan, vhat.pri, vhat.left] = predicted federal deputy vote share in the unit for the current election, a linear projection of the last five races.
\item[win] = winner's party or coalition.
